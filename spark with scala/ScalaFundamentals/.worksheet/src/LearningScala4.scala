object LearningScala4 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(285); 
 ...
 val aBunchOfStuff = ("Kirk", 1964, true);System.out.println("""aBunchOfStuff  : (String, Int, Boolean) = """ + $show(aBunchOfStuff ));$skip(400); 
...
val backwardShips = shipList.map( (ship: String) => {ship.reverse});System.out.println("""backwardShips  : List[String] = """ + $show(backwardShips ));$skip(45); 
 for (ship <- backwardShips) {println(ship)};$skip(184); 
...
val hasThree = iHateThrees.contains(3);System.out.println("""hasThree  : Boolean = """ + $show(hasThree ));$skip(359); 
...
println(shipMap.contains("Archer"));$skip(36); 
println(shipMap.contains("Picard"));$skip(37); 
println(shipMap.contains("Voyager"));$skip(67); 
